...
### 外置 SensitivePromptMasker 插件

SensitivePromptMasker 是完全独立的动态插件项目，源码、测试、tag 和二进制发布均位于 [`Ironboxplus/SensitivePromptMasker`](https://github.com/Ironboxplus/SensitivePromptMasker)。CPA Pro 不内置其源码，也不在 core release 或 Docker 镜像中编译、复制该插件。上层兼容层只通过公开 interceptor metadata 提供稳定的 `cpa_request_id`；未安装插件时 CPA Pro 仍可独立运行。
...
默认配置中的兼容项保持关闭。需要使用时，请从 SensitivePromptMasker release 单独下载对应平台动态库，放入 CPA 配置的插件目录（Docker 推荐通过只读 volume 挂载），再按独立项目 README 启用。core 和插件可以分别升级或回滚。
